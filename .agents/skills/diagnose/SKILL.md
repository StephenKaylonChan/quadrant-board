...
- **重复模式 → lint 建议**（Addy Osmani "重复犯错升级为 lint 规则"理念）：
  - 发现 2+ 次重复的反模式（如多处 `style={{}}`、多处裸 SQL、多处 `@ts-ignore`）→ 标记为"建议升级为 lint 规则或 `.Codex/rules/` 红线"
  - 输出具体 lint 配置建议（如 ESLint `no-restricted-syntax` 规则 / `.Codex/rules/*.md` MUST NOT 条款）
...
1. **读 AGENTS.md "技术栈"段**：识别主要语言（如 "TypeScript + Python"）
...
不进入 Batch 重构，而是输出到报告的**"lint 建议"章节**，让用户自己评估是否加到 ESLint / `.Codex/rules/` 配置。
...
发现以下重复反模式，建议升级为 lint 规则或 `.Codex/rules/` 红线：

| 模式 | 出现次数 | 建议 |
|------|---------|------|
| `style={{}}` | 8 处 | ESLint `no-restricted-syntax` 或 `.Codex/rules/frontend.md` MUST NOT |
| 裸 SQL 字符串 | 5 处 | `.Codex/rules/backend.md` MUST 使用 ORM |
...
**lint 建议**：用户自行评估后加到 ESLint / `.Codex/rules/` 配置（一次投入，长期防御）。
