...
Options:
1. 现在逐一确认（你回答后 Codex 继续整理）
2. 标记为 draft，这些点后续讨论（Spec 里用 TODO 标出）
3. 按 Codex 建议方案写入（自担风险）
```
...
| 类型 | 语法 | 说明 | 示例 |
|------|------|------|------|
| **`[auto: <表达式>]`** | 可观察事实的表达式 | /done 读取文件/spec 验证，**Codex 只读不判断** | `[auto: phase.tasks.unchecked == 0]` |
...
| `implementing` | 实施中 | Codex 基于 spec 开始编码时自动切换 |
