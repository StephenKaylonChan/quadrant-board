...
本机开发不配 `APP_PASSWORD` / `SESSION_SECRET` 时鉴权关闭，免登录，方便开发。两个都配齐才开启登录（首次用 `APP_USERNAME` 默认 `admin` + `APP_PASSWORD` 种子初始账号，之后在 app 内自助改账号）。生产配置和完整方案见 `docs/DEPLOYMENT.md` §2。
...
```text
backend/app/main.py               FastAPI 入口、路由、静态目录和鉴权守卫
backend/app/auth.py               密码哈希、签名 cookie 和 require_auth 守卫
backend/app/routers/auth.py       登录 / 登出 / 状态 / 自助改账号接口
...
backend/app/database.py           async SQLAlchemy 连接、建表、轻量迁移和账号种子
frontend/src/App.tsx              单页应用状态容器
frontend/src/api.ts               前端请求封装（统一带 cookie 和 401 处理）
frontend/src/components/          四象限、散点、卡片、编辑弹窗、AI 输入、灯箱、登录页、账号弹窗和错误边界
