...
- `QuadrantBoard`：按 `important + due_date` 分象限，处理状态/截止日排序、拖拽落点、已完成折叠。
...
1. 已过期任务置顶。
2. `doing` 在普通待办前面。
3. `review` 表示等人处理，仍留在面板。
4. `todo` 按截止日期和 `sort_order` 排序。

`done` 不参与活跃列表，进入每个象限底部的「已完成」折叠区。跨象限拖拽时，前端只提交最小必要字段：`important`、`due_date`、`sort_order`。
