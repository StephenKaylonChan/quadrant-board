...
**已修改文件**（`git status --short` 里 ` M` 开头）：Codex 读取并自动 stage（排除 `.env`、`*.log`、`node_modules/`、构建产物）。
...
**简单改动**（≤3 文件，单一主题）→ Codex 直接生成 Conventional Commits message。
...
选 3 → Codex 按 Tidy First 拆 commit（结构先、行为后，详见 /implement Step 5）。
...
写 6 段 + 关联指针到 `.Codex/session-notes.md`：
...
交接文档：
  .Codex/session-notes.md (6 段 / 2 段 quick)
━━━━━━━━━━━━━━━━━━━━━━━━
