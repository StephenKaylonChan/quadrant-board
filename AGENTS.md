...
- 手动排序用 `sort_order`(float,取前后邻居中间值实现"插队");有期限象限实际显示顺序先按状态/过期和截止日期分层,再用 `sort_order`。`database.py` 的 init_db 里有给老库补列的迁移逻辑,加新列照这个模式做。
...
- 图片交互:卡片缩略图点击=复制剪贴板(`clipboard.ts`,非 PNG 走 canvas 转码);弹窗点图开自制灯箱 `Lightbox.tsx`,灯箱内右键=复制(拦截 contextmenu)。
...
- 任务状态四档:todo / doing / review(PR 待审,不归档)/ done;只有 done 会写 completed_date 进归档。AI 拆解也输出 status(SYSTEM_PROMPT 里有关键词映射),TaskCreate 接受 status。
...
- 测试方式:`curl http://localhost:8000/api/health`;接口文档 http://localhost:8000/docs。
