...
- `frontend/src/components/Lightbox.tsx` 负责图片预览，右键复制走 `clipboard.ts`。
...
- `done` 任务用 `completed_date` 决定历史可见性；`review` 留在面板上。
- 有期限任务的紧急感来自 `due_date` 距离今天的远近，不再需要人工打紧急度分。

## 非直觉决策

- 不建每日快照表：自动结转和历史回放用同一个查询维护，避免重复数据。
- 有期限象限按过期、状态、截止日期和手动顺序共同排序：临近截止的任务自然前移，手动顺序只在同一层内生效。
- AI 只产草稿：模型输出必须经过编辑弹窗人工确认，保存后才入库。
- 图片复制统一走前端剪贴板能力：非 PNG 会先转成 PNG，避免浏览器剪贴板格式限制。
